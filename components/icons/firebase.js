const IconFirebase = () => {
  return (
    <svg
      xmlns="http://www.w3.org/2000/svg"
      xmlnsXlink="http://www.w3.org/1999/xlink"
      width="111"
      height="151"
      viewBox="0 0 111 151"
    >
      <defs>
        <linearGradient
          id="linearGradient-1"
          x1="-3.482%"
          x2="100.833%"
          y1="17.027%"
          y2="92.887%"
        >
          <stop offset="0%" stopColor="#FFF" stopOpacity="0.1"></stop>
          <stop offset="100%" stopColor="#FFF" stopOpacity="0"></stop>
        </linearGradient>
        <path
          id="path-2"
          d="M95.798 33.625a3.436 3.436 0 00-2.336-2.699 3.454 3.454 0 00-3.48.823l-17.71 17.816-14.03-26.51A3.34 3.34 0 0055.25 21.2a3.34 3.34 0 00-2.995 1.854l-7.613 14.492L25.392 1.914c-1.54-2.898-5.903-2.131-6.416 1.108L.498 121.17l49.532 27.704a10.432 10.432 0 0010.01 0L110 121.17 95.798 33.625z"
        ></path>
      </defs>
      <g fill="none" stroke="#a8b2d1" strokeWidth="5">
        <g transform="translate(-664 -1272)">
          <g transform="translate(40 1187)">
            <g transform="translate(114)">
              <g transform="translate(405 1)">
                <g transform="translate(105.193 83.978)">
                  <g>
                    <mask id="mask-3" fill="#fff">
                      <use xlinkHref="#path-2"></use>
                    </mask>
                    <use
                      fill="url(#linearGradient-1)"
                      fillRule="nonzero"
                      xlinkHref="#path-2"
                    ></use>
                    <g mask="url(#mask-3)">
                      <g transform="translate(-6.346 -.16)">
                        <g fill="none" fillRule="evenodd" strokeWidth="1">
                          <path
                            fill="#0a192f"
                            fillRule="nonzero"
                            d="M6.844 121.33L25.408 3.182c.513-3.239 4.876-4.006 6.416-1.108l19.163 35.632L58.6 23.215a3.423 3.423 0 013.037-1.839c1.277 0 2.448.709 3.036 1.839l51.672 98.115H6.844z"
                          ></path>
                          <g transform="translate(0 30.12)">
                            <image
                              width="74.427"
                              height="94.62"
                              x="0"
                              y="0"
                              opacity="0.12"
                              xlinkHref="data:image/png;base64,"
                            ></image>
                            <path
                              fill="#0a192f"
                              fillRule="nonzero"
                              d="M70.8339555 45.1789654L50.901212 7.58665646 6.84386044 91.2103641z"
                            ></path>
                          </g>
                        </g>
                        <g
                          fill="none"
                          fillRule="evenodd"
                          strokeWidth="1"
                          transform="translate(3.422 23.3)"
                        >
                          <image
                            width="119.768"
                            height="131.275"
                            x="0"
                            y="0"
                            opacity="0.2"
                            xlinkHref="data:image/png;base64,"
                          ></image>
                          <g
                            fill="#0a192f"
                            fillRule="nonzero"
                            transform="translate(3.422 7.388)"
                          >
                            <path d="M109.502 90.642L95.3 3.097A3.436 3.436 0 0092.964.4a3.454 3.454 0 00-3.48.823L0 90.642l49.532 27.704a10.432 10.432 0 0010.01 0l49.96-27.704z"></path>
                          </g>
                        </g>
                        <path
                          fill="#FFF"
                          fillOpacity="0.2"
                          fillRule="nonzero"
                          d="M102.145 33.785a3.436 3.436 0 00-2.337-2.699 3.454 3.454 0 00-3.48.823l-17.71 17.816-14.03-26.51a3.34 3.34 0 00-2.993-1.855 3.34 3.34 0 00-2.994 1.855l-7.614 14.491L31.738 2.074c-1.54-2.898-5.902-2.13-6.416 1.108L6.844 121.33h-.086l.086.085.513.256 71.176-71.093 17.794-17.73a3.453 3.453 0 013.48-.824 3.436 3.436 0 012.338 2.698l14.03 86.693.17-.085-14.2-87.545zm-95.13 87.374L25.408 4.035c.513-3.24 4.876-4.007 6.416-1.108l19.163 35.631L58.6 24.067a3.423 3.423 0 013.037-1.838c1.277 0 2.448.708 3.036 1.838l13.688 25.914-71.347 71.178z"
                        ></path>
                        <path
                          fill="#A52714"
                          fillRule="nonzero"
                          d="M66.385 148.181a10.432 10.432 0 01-10.009 0L6.93 120.562l-.085.768 49.532 27.704a10.432 10.432 0 0010.01 0l49.96-27.704-.086-.768-49.875 27.62z"
                          opacity="0.2"
                        ></path>
                      </g>
                    </g>
                    <path
                      d="M-26.8777064 -6.69547618L137.374944 -6.69547618 137.374944 156.971719 -26.8777064 156.971719z"
                      mask="url(#mask-3)"
                    ></path>
                    <path
                      fill="url(#linearGradient-1)"
                      fillRule="nonzero"
                      d="M95.798 33.625a3.436 3.436 0 00-2.336-2.699 3.454 3.454 0 00-3.48.823l-17.71 17.816-14.03-26.51A3.34 3.34 0 0055.25 21.2a3.34 3.34 0 00-2.995 1.854l-7.613 14.492L25.392 1.914c-1.54-2.898-5.903-2.131-6.416 1.108L.498 121.17l49.532 27.704a10.432 10.432 0 0010.01 0L110 121.17 95.798 33.625z"
                      mask="url(#mask-3)"
                    ></path>
                  </g>
                </g>
              </g>
            </g>
          </g>
        </g>
      </g>
    </svg>
  );
}

export default IconFirebase;